    ...
    protected RecipeProvider createRecipeProvider(HolderLookup.Provider registries, RecipeOutput output) {
        return new RecipeProvider(registries, output) {
            ...
            public void buildRecipes() {
                ...
                shaped(RecipeCategory.TOOLS, ProjectEXItems.ALCHEMICAL_BOOKS.get(0).item())
                    .define('H', ProjectEXItems.HIGH_COVALENCE_DUST.item())
                    .define('R', ProjectEXItems.RED_MATTER.item())
                    .define('E', Items.ENDER_PEARL)
                    .define('P', ProjectEXItems.PHILOSOPHERS_STONE.item())
                    .define('B', Items.BOOK)
                    .pattern("HRH").pattern("EBP").pattern("HRH")
                    .unlockedBy("has_red_matter", has(ProjectEXItems.RED_MATTER.item()))
                    .save(output, id("basic_alchemical_book"));
                shaped(RecipeCategory.TOOLS, ProjectEXItems.ALCHEMICAL_BOOKS.get(1).item())
                    .define('B', ProjectEXItems.ALCHEMICAL_BOOKS.get(0).item())
                    .define('O', ProjectEXItems.MAGNUM_STAR_OMEGA.item())
                    .define('E', ProjectEXItems.MAGNUM_STAR_EIN.item())
                    .pattern("EOE").pattern("OBO").pattern("EOE")
                    .unlockedBy("has_basic_book", has(ProjectEXItems.ALCHEMICAL_BOOKS.get(0).item()))
                    .save(output, id("advanced_alchemical_book"));
                shaped(RecipeCategory.TOOLS, ProjectEXItems.ALCHEMICAL_BOOKS.get(2).item())
                    .define('B', ProjectEXItems.ALCHEMICAL_BOOKS.get(1).item())
                    .define('O', ProjectEXItems.COLOSSAL_STAR_OMEGA.item())
                    .define('E', ProjectEXItems.COLOSSAL_STAR_EIN.item())
                    .pattern("EOE").pattern("OBO").pattern("EOE")
                    .unlockedBy("has_advanced_book", has(ProjectEXItems.ALCHEMICAL_BOOKS.get(1).item()))
                    .save(output, id("master_alchemical_book"));
                shaped(RecipeCategory.TOOLS, ProjectEXItems.ALCHEMICAL_BOOKS.get(3).item())
                    .define('B', ProjectEXItems.ALCHEMICAL_BOOKS.get(2).item())
                    .define('S', ProjectEXItems.FINAL_STAR_SHARD.item())
                    .define('F', ProjectEXItems.FINAL_STAR.item())
                    .pattern("SFS").pattern("FBF").pattern("SFS")
                    .unlockedBy("has_master_book", has(ProjectEXItems.ALCHEMICAL_BOOKS.get(2).item()))
                    .save(output, id("arcane_alchemical_book"));
