...
5. If the change also applies to the maintained 26.3 snapshot line, prepare a
   `docs/release-notes/26.3.<patch>.md` companion and create an SSH-signed
   `26.3.<patch>` tag at the same source commit. Do not consider the stable release
   complete until all four snapshot profile jars have been published and verified.
6. Push the tag. `release.yml` cryptographically rejects unsigned, unauthorized,
   off-main, or version-mismatched tags; runs every applicable optional-mod profile,
   rebuilds the jars byte-for-byte, emits SHA-256 checksums and a CycloneDX 1.6 SBOM,
   creates GitHub provenance/SBOM attestations, and publishes the curated release notes.
7. Verify the downloaded runtime with `sha256sum --check SHA256SUMS` and
   `gh attestation verify <jar> --repo tufkan1/ProjectEX`.
8. To promote those exact GitHub bytes, configure the `distribution` environment with
...
9. Announce known issues and support line, then open the next changelog section.
